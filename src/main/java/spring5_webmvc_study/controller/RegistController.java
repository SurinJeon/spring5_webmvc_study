...
public class RegistController {
	...
	public String handleStep2Post(@RequestParam(value = "agree", defaultValue = "false") Boolean agree, RegistRequest registRequest) { // name값 agree로 넘어온 value를 Boolean타입의 agree라는 변수에 넣겠다
...
//		model.addAttribute("registRequest", new RegistRequest()); // 새로운 객체를 만들어서 던져준 다음, step2에서 form태그로 바로 쓸 수 있도록 함
	...
	@PostMapping("/step3")
	public String handleStep3(RegistRequest request, Errors errors) { // 검증 위한 error parameter넣음
...
//		System.out.println(request); << debugging sysout
		...
		new RegistRequestValidator().validate(request, errors);
		if(errors.hasErrors()) {
			return "register/step2"; // 다시 전 화면으로 돌아가기
		}
